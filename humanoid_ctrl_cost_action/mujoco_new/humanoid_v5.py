...
class HumanoidEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release
    ...
    def control_cost(self, action):
        assert (action == self.data.ctrl).all()
        control_cost_old = self._ctrl_cost_weight * np.sum(np.square(self.data.ctrl))
        control_cost = self._ctrl_cost_weight * np.sum(np.square(action))
        breakpoint()
        assert (self.data.ctrl == action).all()
        assert (np.square(self.data.ctrl) == np.square(action)).all()
        assert (np.sum(np.square(self.data.ctrl)) == np.sum(np.square(action))).all()
        assert (control_cost_old == control_cost).all()
        return control_cost
